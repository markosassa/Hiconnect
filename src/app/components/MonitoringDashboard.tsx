...
export default function MonitoringDashboard() {
  ...
  const [jobsLastPage, setJobsLastPage] = useState(1);
  ...
  const fetchDashboard = async () => {
    try {
      const statsRes = await axios.get("/api/monitoring/stats");
  ...
  const fetchJobs = async (page = 1) => {
    try {
      ...
      const res = await axios.get("/api/monitoring/jobs", {
        params: {
          page,
        },
      });
      ...
      setJobsPage(res.data.data.current_page);

      setJobsLastPage(res.data.data.last_page);
  ...
  const fetchAccessLogs = async (page = 1) => {
    try {
      ...
      const res = await axios.get("/api/monitoring/access-logs", {
        params: {
          page,
        },
      });

      setAccessLogs(res.data.data.data);
      ...
      <div className="bg-slate-50 p-4 md:p-8">
        ...
        <div className="bg-white rounded-xl shadow p-6 mb-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-xl font-semibold">Queue Jobs</h2>

              <p className="text-sm text-slate-500">Totale jobs: {jobsTotal}</p>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="px-4 py-2 text-sm font-medium">
                Pagina {jobsPage} di {jobsLastPage}
              </div>
              <button
                onClick={() => goToJobsPage(jobsPage + 1)}
                disabled={jobsPage === jobsLastPage}
                className="
                  flex items-center gap-1
                  px-3 py-2
                  rounded-lg
                ...
                {loadingJobs ? (
                  ...
                  jobs.map((job) => (
                    ...
                    >
                      ...
                      <td className="p-3">{job.started_at}</td>
                    </tr>
                  ))
                )}
              </tbody>
                ...
                {loadingLogs ? (
                  ...
                  accessLogs.map((log) => (
                    <tr key={log.id} className="border-b hover:bg-slate-50">
                      ...
                      <td className="p-3 font-mono text-xs">{log.route}</td>
                      ...
                      <td className="p-3">{log.accessed_at}</td>
                    </tr>
                  ))
                )}
              </tbody>
...
function Card({ title, value, icon }: any) {
  return (
    <div className="bg-white rounded-xl shadow p-6">
      ...
      <div className="text-slate-500 mt-2">{title}</div>
    </div>
  );
}
